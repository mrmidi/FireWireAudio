...
bool appendBytes(std::vector<uint8_t>& vec, uint64_t value, size_t numBytes) {
    for (size_t i = 0; i < numBytes; ++i) {
        vec.push_back(static_cast<uint8_t>((value >> (8 * (numBytes - 1 - i))) & 0xFF));
    }
    return true;
}

std::vector<uint8_t> makeDescriptorSpecifier(
...
)
{
    ...
    switch (type) {
        ...
        case DescriptorSpecifierType::SubunitDependentEnd:
            // Specific data must be appended by caller if needed
...
uint64_t readBytes(const uint8_t* buffer, size_t numBytes) {
    uint64_t val = 0;
    for (size_t i = 0; i < numBytes; ++i) {
        val = (val << 8) | buffer[i];
    }
...
size_t getEffectiveSize(size_t dynamicSize, size_t defaultSize) {
    return dynamicSize == 0 ? defaultSize : dynamicSize;
}

ParsedDescriptorSpecifier parseDescriptorSpecifier(
...
size_t expectedDescriptorSpecifierSize(
    const uint8_t* buffer, 
    size_t bufferLen,
    size_t sizeOfListId,
    size_t sizeOfObjectId,
